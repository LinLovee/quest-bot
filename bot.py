...
║ Версия: 5.2 FIXED (5500+ строк кода) ║

║ Статус: ✅ ЛОКАЦИИ, КЛАСС-СПЕЦИФИЧНОЕ ОРУЖИЕ, ПВП ОЧЕРЕДЬ (ИСПРАВЛЕНО) ║
...
def init_database():
    ...
    # Таблица ПВП очереди (ИСПРАВЛЕНА)
    c.execute("""
...
# ─────────────────────────────────────────────────────────────────────────────
# ⚔️ СИСТЕМА ПВП (ОЧЕРЕДЬ ПОДТВЕРЖДЕНИЯ) - ИСПРАВЛЕНО
# ─────────────────────────────────────────────────────────────────────────────
...
def find_pvp_opponent(chat_id: int, user_id: int) -> Optional[Dict[str, Any]]:
    """✅ ИСПРАВЛЕНО - Найти противника из подтвердивших людей в очереди"""
    player = get_player(chat_id, user_id)
    if not player:
        return None
    ...
    c.execute("""
        ...
        FROM players p
        JOIN pvp_queue q ON p.user_id = q.user_id AND p.chat_id = q.chat_id
        WHERE p.chat_id = ? 
          AND p.user_id != ? 
...
async def select_location_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """✅ Выбрана локация, показываем подтверждение"""
    query = update.callback_query
    user = query.from_user
    chat = query.message.chat
...
async def show_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"""{CLASSES[player["class"]]["emoji"]} {CLASSES[player["class"]]["name"]}!
...
async def show_weapons_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"⚔️ МАГАЗИН ОРУЖИЯ\n\n{CLASSES[player['class']]['name']}:\n\n"

    keyboard = []

    for weapon_id, weapon_info in WEAPONS.items():
        # Пропускаем оружие не для этого класса
        ...
        keyboard.append([InlineKeyboardButton(f"{status} {weapon_info['emoji']} {weapon_info['name']}", callback_data=f"buy_weapon_{weapon_id}")])
...
async def show_armor_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"🛡️ МАГАЗИН БРОНИ\n\n{CLASSES[player['class']]['name']}:\n\n"

    keyboard = []

    for armor_id, armor_info in ARMOR.items():
        # Пропускаем броню не для этого класса
        ...
        keyboard.append([InlineKeyboardButton(f"{status} {armor_info['emoji']} {armor_info['name']}", callback_data=f"buy_armor_{armor_id}")])
...
async def show_pets_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for pet_id, pet_info in PETS.items():
        ...
        text += f"{pet_info['emoji']} {pet_info['name']} - ⚔️ {pet_info['attack_bonus']} 🛡️ {pet_info['defense_bonus']} x{pet_info['xp_bonus']} | 💰 {pet_info['price']}\n"

        keyboard.append([InlineKeyboardButton(f"{status} {pet_info['emoji']} {pet_info['name']}", callback_data=f"buy_pet_{pet_id}")])
...
async def show_runes_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for rune_id, rune_info in RUNES.items():
        ...
        keyboard.append([InlineKeyboardButton(f"{status} {rune_info['emoji']} {rune_info['name']}", callback_data=f"buy_rune_{rune_id}")])
...
async def craft_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("🔨 КРАФТИНГ", callback_data="crafting")]]

    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))

async def show_pvp_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """✅ ИСПРАВЛЕНО - Показать ПВП меню"""
    query = update.callback_query
    user = query.from_user
    chat = query.message.chat
...
Готов к битве один на один?

Найди противника и сражайся!

👥 Минимум 2 игрока должны быть в поиске.
        """
...
async def pvp_check_match(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """✅ ИСПРАВЛЕНО - Проверяем есть ли противник"""
    query = update.callback_query
    user = query.from_user
    chat = query.message.chat
...
Попробуй через несколько секунд.
        ...
        opponent_player = get_player(chat.id, opponent['user_id'])
        text = f"""
⚔️ ПВП АРЕНА

🎉 ПРОТИВНИК НАЙДЕН!

{CLASSES[opponent_player['class']]['emoji']} {opponent['username']} - Ур. {opponent['level']}
...
Начинаем бой!
    ...
    # FIX: Проверяем, изменилось ли сообщение перед редактированием
    try:
        if query.message.text != text:
            await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        else:
            # Не изменилось, просто даём ответ
            await query.answer("⏳ Статус не изменился. Ждём...", show_alert=False)
    except Exception as e:
        logger.error(f"❌ Ошибка при обновлении сообщения: {e}")
        await query.answer("⏳ Проверяем статус...", show_alert=False)
...
Готов к испытанию?
    ...
    if is_active:
        ...
        keyboard = [
            [InlineKeyboardButton("⚔️ НАЧАТЬ", callback_data="dungeon_start")],
            [InlineKeyboardButton("⬅️ ГЛАВНОЕ МЕНЮ", callback_data="main_menu")],
        ]
...
async def show_ratings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🌍 ГЛОБАЛЬНЫЙ РЕЙТИНГ", callback_data="rating_global")],
        [InlineKeyboardButton("⚔️ ПВП РЕЙТИНГ", callback_data="rating_pvp")],
        [InlineKeyboardButton("🏆 РЕЙТИНГ ПОДЗЕМЕЛЬЯ", callback_data="rating_dungeon")],
        [InlineKeyboardButton("⬅️ ГЛАВНОЕ МЕНЮ", callback_data="main_menu")],
    ]
...
async def show_global_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if player:
        text += f"\n\n👤 Ты на {player_position} месте\n"
        text += f"⭐ Уровень: {player['level']}\n"
        text += f"💰 Золото: {player['gold']}\n"

    keyboard = [[InlineKeyboardButton("📊 РЕЙТИНГИ", callback_data="ratings")]]
...
async def show_pvp_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, leader in enumerate(leaders, 1):
        ...
        text += f"{medal} {leader['username']} - Ур. {leader['level']} - {wins}W {losses}L {winrate}%\n"
...
async def show_dungeon_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, leader in enumerate(leaders, 1):
        ...
        text += f"{medal} {leader['username']} - Ур. {leader['level']} - Этаж {leader['dungeon_rating']} | 👹 {leader['total_bosses_killed']}\n"
...
def main():
    ...
    logger.info("✅ RuneQuestRPG BOT v5.2 READY!")
