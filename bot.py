...
def find_pvp_opponent(chat_id: int, user_id: int) -> Optional[Dict[str, Any]]:
    """Найти противника из подтвердивших людей в очереди (глобально)"""
    player = get_player(chat_id, user_id)
    if not player:
        return None
    ...
    # ищем подтвердивших противников ±5 уровней, но не самого себя, глобально
    min_level = max(1, player["level"] - 5)
    max_level = player["level"] + 5

    c.execute("""
        SELECT p.user_id, p.username, p.level, p.attack, p.defense, p.gold, p.chat_id
        FROM players p
...
def pvp_battle(chat_id: int, attacker_id: int, defender_id: int) -> Dict[str, Any]:
    """Симулировать PvP бой"""
    attacker = get_player(chat_id, attacker_id)  # chat_id атакующего
    defender_chat_id = get_player_chat_id(defender_id)  # Нужно получить chat_id defender
    ...
    attacker_damage, attacker_crit = calculate_damage(
        attacker_stats["attack"], defender_stats["defense"], attacker_stats["crit_chance"], attacker_stats["spell_power"]
    )

    # Атака защитника
    defender_damage, defender_crit = calculate_damage(
        defender_stats["attack"], attacker_stats["defense"], defender_stats["crit_chance"], defender_stats["spell_power"]
    )

    if attacker_damage > defender_damage:
        winner_id = attacker_id
        reward_gold = int(defender["gold"] * 0.1)
        subtract_gold(defender_chat_id, defender_id, reward_gold)
        add_gold(chat_id, attacker_id, reward_gold)
        update_pvp_stats(attacker_id, True)
        update_pvp_stats(defender_id, False)
    else:
        winner_id = defender_id
        reward_gold = int(attacker["gold"] * 0.1)
        subtract_gold(chat_id, attacker_id, reward_gold)
        add_gold(defender_chat_id, defender_id, reward_gold)
        update_pvp_stats(defender_id, True)
        update_pvp_stats(attacker_id, False)

    # Удаляем обоих из очереди
    cancel_pvp_search(chat_id, attacker_id)
    cancel_pvp_search(defender_chat_id, defender_id)
...
@safe_db_execute
def get_player_chat_id(user_id: int) -> int:
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT chat_id FROM players WHERE user_id = ?", (user_id,))
    row = c.fetchone()
    conn.close()
    return row["chat_id"] if row else None

@safe_db_execute
def update_pvp_stats(user_id: int, win: bool):
    conn = get_db()
    c = conn.cursor()
    if win:
        c.execute("UPDATE players SET pvp_wins = pvp_wins + 1 WHERE user_id = ?", (user_id,))
    else:
        c.execute("UPDATE players SET pvp_losses = pvp_losses + 1 WHERE user_id = ?", (user_id,))
    conn.commit()
    conn.close()
...
def get_global_leaderboard(chat_id: int, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    c = conn.cursor()
    c.execute(
        "SELECT username, level, gold FROM players ORDER BY level DESC, gold DESC LIMIT ?",
        (limit,)
    )
...
def get_pvp_leaderboard(chat_id: int, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    c.execute(
        ...
        FROM players 
        ORDER BY pvp_wins DESC, win_rate DESC LIMIT ?
        """,
        (limit,)
    )
    leaders = [dict(row) for row in c.fetchall()]
...
def get_dungeon_leaderboard(chat_id: int, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    c = conn.cursor()
    c.execute(
        "SELECT username, level, dungeon_rating, total_bosses_killed FROM players ORDER BY dungeon_rating DESC LIMIT ?",
        (limit,)
    )
    leaders = [dict(row) for row in c.fetchall()]
    conn.close()
...
def get_player_position(chat_id: int, user_id: int) -> int:
    ...
    c.execute(
        ...
        FROM players 
        WHERE (level > (SELECT level FROM players WHERE user_id = ? AND chat_id = ?)) 
           OR (level = (SELECT level FROM players WHERE user_id = ? AND chat_id = ?) AND gold > (SELECT gold FROM players WHERE user_id = ? AND chat_id = ?))
        """,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = """
🎮 ДОБРО ПОЖАЛОВАТЬ В RUNEQUESTRPG!
...
Выбери класс героя:
    ...
    for class_id, class_info in CLASSES.items():
        keyboard.append([InlineKeyboardButton(f"{class_info['emoji']} {class_info['name']} - {class_info['description']}", callback_data=f"class_{class_id}")])
...
async def select_class(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    init_player(chat.id, user.id, user.username, class_id)
...
Выбери действие:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🏞️ ЛОКАЦИИ", callback_data="locations_list"), InlineKeyboardButton("⚔️ ПВП", callback_data="pvp_menu")],
        [InlineKeyboardButton("🛍️ МАГАЗИН", callback_data="shop"), InlineKeyboardButton("⚙️ ЭКИПИРОВКА", callback_data="equipment")],
        [InlineKeyboardButton("🔨 КРАФТИНГ", callback_data="crafting"), InlineKeyboardButton("📊 РЕЙТИНГИ", callback_data="ratings")],
    ]
...
async def show_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    xp_needed = int(LEVEL_UP_BASE * (player["level"] ** 1.5))
    xp_percent = int((player["xp"] / xp_needed) * 100) if player["level"] < MAX_LEVEL else 100
...
async def pvp_check_match(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not opponent:
        await query.answer("❌ Противник не найден. Продолжаем поиск...", show_alert=True)
        return

    # Когда найден, редактируем
    text = f"""
⚔️ ПВП АРЕНА

🎉 ПРОТИВНИК НАЙДЕН!

{CLASSES[opponent['class']]['emoji']} {opponent['username']} - Ур. {opponent['level']}
...
async def pvp_start_fight(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    defender = get_player(get_player_chat_id(defender_id), defender_id)
